...
namespace FeTracker.Sni.Services
{
    public static class GrpcChannelService
    {
        ...
        /// <param name="host">Defaults to localhost. Used to allow talking to SNI running on another machine in the network. Currently untested. Per docs, if an FxPak is used, must be localhost.</param>
        /// <param name="useHttpHandler">Defaults to true. Use false for a desktop application</param>
        /// <returns></returns>
        public static GrpcChannel? GetChannel(int port, string host, bool useHttpHandler) => useHttpHandler
